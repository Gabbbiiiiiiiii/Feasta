...
import {
  ...
  createCustomerPhoneRecaptcha,
  requestCustomerPhoneVerification,
  type CustomerPhoneVerificationSession,
} from "@/lib/auth/customer-phone-client";
...
/*
 * Client-side countdown UX.
 *
 * Firebase ultimately remains authoritative for whether
 * a verification session/code is still valid.
 */
const OTP_EXPIRY_SECONDS =
  5 * 60;

export default function CustomerPhoneVerificationForm({
  initialPhoneNumber,
  returnTo,
}: {
  ...
  const [
    ...
    useState<
      CustomerPhoneVerificationSession |
      null
    >(null);
  ...
  const verifierRef =
    useRef<
      ReturnType<
        typeof createCustomerPhoneRecaptcha
      > |
      null
    >(null);
  ...
  /*
   * reCAPTCHA must be cleared when the component leaves
   * the page so Firebase doesn't retain a stale verifier.
   */
  useEffect(() => {
    return () => {
      verifierRef.current
  ...
  /*
   * Focus the first OTP field when an SMS session
   * becomes available.
   */
  useEffect(() => {
    if (
  ...
  async function sendCode(
    ...
    try {
      ...
      /*
       * Prevent rapid resend spam from the UI.
       *
      ...
      console.error(
        "[FEASTA customer phone verification] Failed to send verification code:",
        caught,
      );
  ...
  async function verifyCode(
    ...
    try {
      const result =
        await confirmCustomerPhoneVerification(
          session,
          code,
          returnTo,
        );
      ...
      setMessage(
        "Your mobile number is verified. Returning to your booking.",
      );
      ...
      console.error(
        "[FEASTA customer phone verification] Failed to verify code:",
        caught,
      );
  ...
  function updateOtpDigit(
    ...
    /*
     * Some browsers/password managers can place all six
     * digits in the focused field.
     */
    if (
      digits.length > 1
    ) {
          ...
          <div className="rounded-dialog border border-border bg-card px-5 py-7 shadow-floating sm:px-8 sm:py-9">
            ...
            <div className="mx-auto mt-6 max-w-xl text-center">
              ...
              <p className="mt-4 leading-7 text-muted-foreground">
                ...
                verify a Philippine mobile number that FEASTA
                can use for booking-related communication.
              </p>
            ...
            {editingNumber ? (
              ...
              >
                <label
                  htmlFor="customer-replacement-phone"
                ...
                <p className="text-sm leading-6 text-muted-foreground">
                  Enter the mobile number you want associated
                  with your FEASTA customer account.
                </p>
              ...
              >
                <div>
                  ...
                  <div
                    ...
                    role="group"
                    aria-labelledby="customer-phone-code-label"
                    aria-describedby="customer-phone-code-help"
                  >
                    {otpDigits.map(
                      (
                        digit,
                        ...
                        <input
                          ...
                          id={`customer-phone-code-${index + 1}`}
                          type="text"
                          inputMode="numeric"
                          autoComplete={
                            index ===
            ...
            <div className="mt-6 rounded-card border border-primary/15 bg-secondary p-5">
              <div className="flex items-start gap-3">
                ...
                <p className="text-sm leading-6 text-muted-foreground">
                  FEASTA uses your verified mobile number
                  for important booking updates and account
                  security. Your verification state is confirmed
...
function customerLoginDestination(
  returnTo: string,
): string {
  const parameters =
    new URLSearchParams({

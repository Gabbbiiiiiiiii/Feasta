...
export function providerOnboardingError(
  error: unknown,
): string {
  const code =
    typeof error === "object" &&
  ...
  if (
    ...
    if (
      ...
      return (
        "Your provider application has already been created. " +
        "Changes must be saved to your existing application instead of creating it again."
      );
    }
    ...
    if (
      ...
      return (
        "Complete your service selection before saving capacity and schedule."
      );
    }
    ...
    if (
      ...
      return (
        "Read and accept the FEASTA Provider Agreement before continuing."
      );
    }

    return (
      message ||
      "This provider application cannot be updated from its current state."
    );
  }
  ...
  if (
    ...
    return (
      message ||
      "Review your provider information and correct the invalid fields."
    );
  }
  ...
  if (
    ...
    return (
      "Your provider session cannot make this change. " +
  ...
  if (
    ...
    return (
      "The provider application could not be found. Refresh the page and try again."
    );
  }

  // Plain Error instances here are generated by FEASTA's own
  // onboarding client validation/navigation code. Preserve these
  // actionable messages instead of replacing them with the generic
  ...
  return customerAuthenticationError(
    error,
  );
}

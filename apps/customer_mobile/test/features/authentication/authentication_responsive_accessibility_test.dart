...
void main() {
  for (final width in _authenticationWidths) {
    testWidgets(
      'authentication surfaces remain usable at ${width.toInt()} px and 200 percent text',
      (tester) async {
...
Widget _largeTextApp(Widget home) => MediaQuery(
...
class _ResponsiveLoginGateway implements CustomerLoginGateway {
  ...
  }) async => const CustomerLoginResult(
    uid: 'customer-one',
  ...
  Future<CustomerLoginResult> signInWithGoogle() =>
      signInWithEmail(email: 'customer@feasta.test', password: '');
}

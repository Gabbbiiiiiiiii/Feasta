...
const normalizeLookupValue = (value) => value
    ...
    .replace(/\s+/gu, " ")
    .replace(/\s*,\s*/gu, ",");
const SERVICE_AREA_LOOKUP = new Map(PROVIDER_SERVICE_AREAS.flatMap((area) => {
    const values = [area.key, area.label, area.city, ...area.aliases];
    return values.flatMap((value) => [
        [normalizeLookupValue(value), area],
        [normalizeLookupValue(`${value}, ${area.province}`), area],
    ]);
}));
/**
 ...
 */
export function normalizeProviderServiceArea(value, province) {
...
export function normalizeExistingProviderServiceAreas(value) {
...
//# sourceMappingURL=provider-service-area.js.map
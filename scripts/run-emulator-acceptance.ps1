  ...
  switch ($Suite) {
    ...
    "payment" {
      & pnpm.cmd exec firebase emulators:exec `
        ...
        --project demo-feasta-phase3 `
        --only firestore `
        "node --test --test-concurrency=1 functions/test/emulator/payment-checkout.integration.cjs functions/test/emulator/payment-webhook.integration.cjs functions/test/emulator/payment-lifecycle-p7.integration.cjs"
      $exitCode = $LASTEXITCODE
    }
  }
} finally {
